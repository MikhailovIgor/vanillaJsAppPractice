...
import {closeModal, addProduct, editProduct} from "../redux/actions.js";
...
const modal = (product) => {
  ...
  `);
    ...
    <div class="form__container">
          <label for="deviceModel" class="form__label form__label--required">Enter device model:</label>
          <input id="deviceModel" class="form__input" type="text" value=${product.title} required>
          <label for="imgUrl" class="form__label form__label--required">Enter picture address:</label>
          <input id="imgUrl" class="form__input" type="text" value=${product.image}>
          <div class="form__prices">
            <div class="form__column">
              <label for="price" class="form__label form__label--required">Enter price:</label>
              <input id="price" class="form__input" type="number" value=${product.price} min="1" step="0.01" required>
            </div>
            <div class="form__column">
              <label for="discount" class="form__label">Discount:</label>
              <input id="discount" class="form__input" type="number" value=${product.discount} min="1" max="99" step="1">
            </div>
          </div>
          <label for="description" class="form__label form__label--required">Enter description:</label>
          <textarea id="description" rows="5" maxlength="180" required>${product.description}</textarea>

          <div class="form__footer">
            <div class="form__column">
              <div class="form__row">
                <input id="isHot" class="form__customCheckbox" type="checkbox" ${product.labels.hot ? 'checked' : ''}>
                <label for="isHot" class="form__checkboxLabel">hot</label>
              </div>
              <div class="form__row">
                <input id="isNew" class="form__customCheckbox" type="checkbox" ${product.labels.new ? 'checked' : ''}>
                <label for="isNew" class="form__checkboxLabel">new</label>
              </div>
            </div>
            <button class="form__submit" type="submit">EDIT CARD</button>
          </div>
        </div>
  `);
